...
implements AutoCloseable {
    ...
    public TagStringWriter writeTag(BinaryTag binaryTag) {
    ...
    private TagStringWriter writeCompound(CompoundBinaryTag compoundBinaryTag) {
        this.beginCompound();
    ...
    private TagStringWriter writeList(ListBinaryTag listBinaryTag) {
        this.beginList();
        int n = 0;
    ...
    private TagStringWriter writeByteArray(ByteArrayBinaryTag byteArrayBinaryTag) {
        if (this.legacy) {
    ...
    private TagStringWriter writeIntArray(IntArrayBinaryTag intArrayBinaryTag) {
        if (this.legacy) {
            this.beginList();
        } else {
            this.beginArray('i');
    ...
    private TagStringWriter writeLongArray(LongArrayBinaryTag longArrayBinaryTag) {
        if (this.legacy) {
            throw new IOException("Legacy Mojangson only supports integer arrays!");
        }
        this.beginArray('l');
    ...
    public TagStringWriter value(String string, char c) {
        if (c == '\u0000') {
    ...
    public TagStringWriter endList(boolean bl) {
        --this.level;
        if (bl) {
            this.newlineIndent();
        }
    ...
    private TagStringWriter beginArray(char c) {
    ...
    private void writeMaybeQuoted(String string, boolean bl) {
        if (!bl) {
    ...
    private void printAndResetSeparator(boolean bl) {
    ...
    private Appendable appendSeparator(char c) {
        this.out.append(c);
        if (this.prettyPrinting()) {
            this.out.append(' ');
        }
