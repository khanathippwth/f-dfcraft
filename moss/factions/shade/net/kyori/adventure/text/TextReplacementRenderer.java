...
implements ComponentRenderer<State> {
    ...
    public Component render(@NotNull Component component, @NotNull State state) {
        ...
        Object object;
        int n;
        ArrayList<TranslationArgument> arrayList;
        ...
        if (component instanceof TextComponent) {
            ...
            hoverEvent = ((TranslatableComponent)component2).arguments();
            arrayList = null;
            int n3 = hoverEvent.size();
            for (n = 0; n < n3; ++n) {
                TranslationArgument translationArgument;
                componentLike = (TranslationArgument)hoverEvent.get(n);
                ComponentLike componentLike2 = translationArgument = componentLike.value() instanceof Component ? TranslationArgument.component(this.render((Component)componentLike.value(), state)) : componentLike;
                if (translationArgument != componentLike && arrayList == null) {
                    arrayList = new ArrayList<TranslationArgument>(n3);
                    if (n > 0) {
                        arrayList.addAll(hoverEvent.subList(0, n));
                    }
                }
                if (arrayList == null) continue;
                arrayList.add(translationArgument);
            }
            if (arrayList != null) {
                component2 = ((TranslatableComponent)component2).arguments((List<? extends ComponentLike>)arrayList);
            }
        }
        if (state.running) {
    ...
    private static /* synthetic */ void lambda$render$0(HoverEvent hoverEvent, Style.Builder builder) {
